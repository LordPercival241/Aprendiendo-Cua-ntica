...
export function DoubleSlitSimulator() {
  ...
  // L is in m, λ in nm and d in μm. Their numerical ratio is already mm:
  // Δy[mm] = L[m] × λ[nm] / d[μm].
  const fringeMm = distance * wavelength / separation;
...
export function StateVectorSimulator() {
  const [polarAngle, setPolarAngle] = useState(62);
  const [relativePhase, setRelativePhase] = useState(45);
  const theta = polarAngle * Math.PI / 180;
  const phi = relativePhase * Math.PI / 180;
  const c0 = Math.cos(theta / 2);
  const c1 = Math.sin(theta / 2);
  const probability0 = c0 ** 2;
  const probability1 = c1 ** 2;
  const sigmaX = Math.sin(theta) * Math.cos(phi);
  const sigmaY = Math.sin(theta) * Math.sin(phi);
  const sigmaZ = Math.cos(theta);

  return <LabShell title="Estado de dos niveles y operadores" subtitle="Construye un vector de estado normalizado en una base ortonormal. La fase relativa no cambia las probabilidades en la base elegida, pero sí modifica los valores esperados de operadores no diagonales.">
    <div className="grid gap-5 lg:grid-cols-[1fr_280px]"><div className="rounded-2xl border border-zinc-800 bg-zinc-950 p-4"><svg viewBox="0 0 560 300" className="h-[280px] w-full" role="img" aria-label="Representación de un vector de estado de dos niveles"><line x1="80" y1="230" x2="500" y2="230" stroke="#52525b"/><line x1="80" y1="42" x2="80" y2="230" stroke="#52525b"/><rect x="155" y={230 - probability0 * 150} width="95" height={probability0 * 150} fill="#22d3ee" fillOpacity="0.75"/><rect x="340" y={230 - probability1 * 150} width="95" height={probability1 * 150} fill="#818cf8" fillOpacity="0.75"/><text x="165" y="258" fill="#e4e4e7" fontSize="14">|0⟩</text><text x="350" y="258" fill="#e4e4e7" fontSize="14">|1⟩</text><text x="155" y="30" fill="#a1a1aa" fontSize="13">Probabilidades de la base elegida</text><text x="155" y={220 - probability0 * 150} fill="#67e8f9" fontSize="13">{probability0.toFixed(3)}</text><text x="340" y={220 - probability1 * 150} fill="#a5b4fc" fontSize="13">{probability1.toFixed(3)}</text></svg></div><div className="grid gap-3"><RangeControl label="Ángulo de mezcla θ" value={polarAngle} min={0} max={180} step={1} unit="°" onChange={setPolarAngle}/><RangeControl label="Fase relativa φ" value={relativePhase} min={0} max={360} step={1} unit="°" onChange={setRelativePhase}/><Metric label="Norma">{(probability0 + probability1).toFixed(3)}</Metric></div></div>
    <div className="grid gap-3 sm:grid-cols-3"><Metric label="⟨σₓ⟩">{sigmaX.toFixed(3)}</Metric><Metric label="⟨σᵧ⟩">{sigmaY.toFixed(3)}</Metric><Metric label="⟨σ_z⟩">{sigmaZ.toFixed(3)}</Metric></div>
    <LaboratoryModel title="Espacio de Hilbert de dimensión dos" phenomenon="El laboratorio verifica la normalización, la regla de Born y cómo la fase relativa se vuelve observable al medir operadores no diagonales en la base elegida." equations={[{ label: 'Estado', math: String.raw`|\psi\rangle=\cos\frac\theta2|0\rangle+e^{i\phi}\sin\frac\theta2|1\rangle`, explanation: 'La parametrización garantiza que el estado está normalizado.' }, { label: 'Regla de Born', math: String.raw`P_j=|\langle j|\psi\rangle|^2,\qquad P_0+P_1=1`, explanation: 'Las barras muestran las probabilidades en la base {|0⟩,|1⟩}.' }, { label: 'Valor esperado', math: String.raw`\langle\hat A\rangle=\langle\psi|\hat A|\psi\rangle`, explanation: 'Los valores de las matrices de Pauli dependen de θ y φ.' }]} assumptions="Espacio de Hilbert de dos dimensiones y estado puro. Es una herramienta para la notación de Dirac, no una dinámica de Rabi ni una simulación relativista." />
  </LabShell>;
}
...
export function SternGerlachSimulator() {
  ...
  const [coupledState, setCoupledState] = useState<'tripletPlus' | 'tripletZero' | 'tripletMinus' | 'singlet'>('tripletZero');
  ...
  const coupledStates = { tripletPlus: { label: '|1,1⟩', expansion: '|↑↑⟩', description: 'Triplete con proyección total máxima.' }, tripletZero: { label: '|1,0⟩', expansion: '(|↑↓⟩ + |↓↑⟩)/√2', description: 'Combinación simétrica con proyección total cero.' }, tripletMinus: { label: '|1,−1⟩', expansion: '|↓↓⟩', description: 'Triplete con proyección total mínima.' }, singlet: { label: '|0,0⟩', expansion: '(|↑↓⟩ − |↓↑⟩)/√2', description: 'Singlete antisimétrico de espín total cero.' } } as const;
  return <LabShell title="Stern–Gerlach secuencial" subtitle="Un haz preparado en |+z⟩ cruza un primer analizador, se filtra su salida +n₁ y luego entra en un segundo analizador. Cambia los ejes para observar proyección y preparación de estado.">
    ...
    <section className="rounded-2xl border border-zinc-800 bg-zinc-950 p-5"><p className="text-[10px] font-mono font-semibold uppercase tracking-[0.16em] text-cyan-400">Composición de dos espines 1/2</p><div className="mt-4 flex flex-wrap gap-2">{(Object.keys(coupledStates) as Array<keyof typeof coupledStates>).map((key) => <button key={key} type="button" onClick={() => setCoupledState(key)} className={`rounded-lg border px-3 py-2 font-mono text-xs transition-colors ${coupledState === key ? 'border-cyan-400 bg-cyan-500/15 text-cyan-200' : 'border-zinc-700 bg-black text-zinc-400 hover:text-white'}`}>{coupledStates[key].label}</button>)}</div><div className="mt-4 grid gap-3 sm:grid-cols-[180px_1fr]"><div className="rounded-xl border border-zinc-800 bg-black p-4 font-mono text-lg text-cyan-200">{coupledStates[coupledState].label}</div><p className="text-sm leading-6 text-zinc-300"><strong className="font-mono text-cyan-300">{coupledStates[coupledState].expansion}</strong><br />{coupledStates[coupledState].description}</p></div></section>
    <LaboratoryModel title="Medición secuencial y composición de espines" phenomenon="El primer analizador prepara el estado filtrado para la medición posterior. La sección adicional construye los estados acoplados permitidos de dos sistemas de espín 1/2." equations={[{ label: 'Primer analizador', math: String.raw`P(+\mathbf n_1|+z)=\cos^2\frac{\theta_1}{2}`, explanation: 'Probabilidad de que el filtro permita pasar el primer canal.' }, { label: 'Segundo analizador', math: String.raw`P(+\mathbf n_2|+\mathbf n_1)=\cos^2\frac{\theta_2-\theta_1}{2}`, explanation: 'El estado ha sido preparado por la primera medición.' }, { label: 'Estados acoplados', math: String.raw`|1,0\rangle=\frac{|\uparrow\downarrow\rangle+|\downarrow\uparrow\rangle}{\sqrt2},\quad |0,0\rangle=\frac{|\uparrow\downarrow\rangle-|\downarrow\uparrow\rangle}{\sqrt2}`, explanation: 'Coeficientes de Clebsch–Gordan para 1/2 ⊗ 1/2.' }]} assumptions="Medición ideal de espín 1/2, detectores perfectos y sin fases dinámicas entre analizadores. Los ángulos se toman en un mismo plano; la composición se limita al caso 1/2 ⊗ 1/2." />
  </LabShell>;
}
